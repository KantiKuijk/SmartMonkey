...
function attributeObjectToString(att) {
    return `[${att.name}${att.value ? "=" + att.value : ""}]`;
}
export function attributes2Emmet(atts) {
    if (typeof atts === "string") {
        //! the regex makes sure of it
        if (htmlAttrRE.test(atts)) {
            return `${atts.startsWith("[") ? "" : "["}${atts}${atts.endsWith("]") ? "" : "]"}`;
        }
        else
            throw new Error("Invalid attribute string: " + atts);
    }
    if (Array.isArray(atts)) {
        return atts
            .map((att) => typeof att === "string"
...
export function applyAttributes(elem, atts) {
     ...
     */
    if (typeof atts === "string")
        atts = attributeStringToObject(atts);
    if (Array.isArray(atts)) {
        atts.forEach((att) => {
            if (typeof att === "string")
                att = attributeStringToObject(att);
...
export function isTextNodeEmmet(emmet) {
    return emmet[0] === "{";
}
export function isNodeListEmmet(emmet) {
    return emmet[0] === "(";
}
export function emmet(strings, ...emmetsOrElems) {
    // keep special characters for ourselves to consume, make mutable
    const baseEmmet = [...strings.raw];
    if (baseEmmet.length === 0)
        throw new Error("Emmet: No emmet given");
    ...
    if (baseEmmet.some((bEmmet) => bEmmet === undefined))
        throw new Error("Emmet: Cannot have consecutive pre-existing elements without emmet inbetween");
    const choppeds = baseEmmet.map((emmet) => emmet.replace(/\n\s*/g, "").split(/(?=[#.{|[()+>)|\\])/gm));
            ...
            if (typeof chop === "string") {
                ...
                if (lastIsCurly) {
                    if (closingCurlyIndex !== -1)
                        throw new Error(`Emmet: Unbalanced ${opening}${closing} brackets, ${name} brackets cannot be nested`);
                ...
                throw new Error(`Emmet: ${name} brackets ${opening}${closing} can only contain text`);
        }
        if (closingCurlyIndex !== -1)
            throw new Error(`Emmet: Unbalanced ${opening}${closing} brackets, ${name} brackets cannot be nested`);
    });
 ...
 * @returns Text Node
 */
export function makeEmmetText(emmetText) {
    return document.createTextNode(emmetText[0] === "{"
        ? emmetText.slice(1, -1).replace(/\\([[\](){}|\\])/g, "$1")
        : emmetText);
}
function emmetCore(chopped) {
    chopped = chopped.filter((x) => Boolean(x));
    const zeroChop = chopped.shift();
    if (!zeroChop)
        throw new Error("Emmet: no content provided for emmet");
    if (typeof zeroChop === "string" && zeroChop[0] === "|")
        throw new Error("Emmet: Invalid syntax, cannot begin with operator (|), consider using round brackets");
    ...
    while (chopped.includes("|")) {
        ...
        const startIndex = chopped.findIndex((chop) => typeof chop === "string" && chop.startsWith("|"));
        const endIndex = chopped.findIndex((chop, i) => chop === "|" && i > startIndex);
        if (startIndex === -1)
        ...
        chopped.splice(startIndex, 1, chopped[startIndex].replace("|", "("));
    ...
    if (typeof zeroChop === "string") {
        ...
        switch (chip) {
            case ">":
            case "+":
                throw new Error(`Emmet: Invalid syntax, cannot begin with operator (${chip})`);
            case ")":
        ...
        if (typeof chop === "string") {
            ...
            switch (chip) {
                ...
                case "{":
                    // application
                    if (!chap)
                        throw new Error("Emmet: Empty applications not allowed");
                    switch (chip) {
                        case ".":
                ...
                case "+":
                    ...
                    if (chap) {
                        ...
                        throw new Error("Emmet: Invalid syntax, operator cannot be last character");
                    }
                        ...
                        switch (nextChip) {
                            ...
                            case "(":
                                ...
                                break;
                            default:
                                throw new Error(`Emmet: Invalid syntax, expected NodeList or Text but got other emmet (${nextChop})`);
                        }
                    }
                    else {
                        subject = nextChop;
                    ...
                    switch (chip) {
                        case ">":
                            if (subject instanceof Text)
                                throw new Error("Emmet: Invalid syntax, Text nodes should be added without > operator");
                        ...
                        case "+":
                            if (!parent)
                                throw new Error("Emmet: cannot add siblings to top-level element, consider surrounding with () to make a NodeList");
            ...
            if (chop instanceof Text)
                cwe.appendChild(chop);
            else
                throw new Error("Emmet: Invalid syntax, missing operator for pre-existing element");
        }
    }

...
import { PluginMain } from "./Plugins.js";
...
const plugin = new PluginMain({
    ...
    activate: () => {
        ...
        if (/^https:\/\/[a-zA-Z]+\.smartschool\.be\/\?module=Profile$/.test(href)) {
            ...
            if (!smscSettings)
                return;
            const existingSettingsLink = smscSettings.querySelector(".smscAdminNav_body_link");
            ...
            {
                const title = settingsLink.querySelector(".smscAdminNav_body_linkdivTitle");
                if (title) {
                ...
                const image = settingsLink.querySelector("div.smscAdminNav_body_linkdivIcon");
                if (image) {
          ...
          dialog#smk-settings
            ...
            +${(() => {
                        return array2NodeList(PLUGINIDS.filter((pid) => !ALWAYSENABLEDPLUGINS.includes(pid)).map((pid) => {
